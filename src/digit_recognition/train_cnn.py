...
from src.digit_recognition.CNN import digit_recognition, digit_recognition_GAP, digit_recognition_new
...
def train(
    ...
    image_train_dataset = ImageDataset(size=3000)
    image_test_dataset = ImageDataset(size=100)
    mix_dataset = ConcatDataset([image_train_dataset, get_mnist()])
    batches = DataLoader(
        dataset=mix_dataset, 
        batch_size=64,
        shuffle=True,
