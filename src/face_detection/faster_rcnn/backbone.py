...
class BackboneVGG16(FasterRCNNBackbone):
    def __init__(self, featmap_list: list[str] = ["c2", "c3", "c4", "c5"]):
        super().__init__()
        self.featmap_list = featmap_list
        self.out_channels=256
...
class ResNet18(nn.Module):
    """
        MAP = [c1, c2, c3, c4] = [64, 128, 256, 512]
    """
    MAP = {
        "4" : "c1", 
        "5" : "c2", 
    ...
    def __init__(self, featmap_list: list[str]):
        ...
        self.features = tc.nn.Sequential(*list(self.features.children())[:-2])
        self.features.requires_grad_(False)
        for params in self.features[7].parameters():
            params.requires_grad_(True)
        for params in self.features[6].parameters():
            params.requires_grad_(True)
        for params in self.features[5].parameters():
            params.requires_grad_(True)
        self.featmap_list = featmap_list
        self.in_channels_list = [self.IN_CHANNELS_LIST[k] for k in featmap_list]
    ...
    def forward(self, X: tc.Tensor):
        ...
        for i, layer in enumerate(self.features):
            X = layer(X)
            if(self.MAP.get(f"{i}", "-1") in self.featmap_list):
                res[self.MAP[f"{i}"]] = X
        return res

class BackboneResNet18(FasterRCNNBackbone):
    def __init__(self, featmap_list: list[str] = ["c1", "c2", "c3", "c4"]):
        super().__init__()
        self.out_channels=256
        self.featmap_list = featmap_list
        self._backbone = ResNet18(featmap_list=featmap_list)
        self._fpn = FeaturePyramidNetwork(in_channels_list=self._backbone.in_channels_list, out_channels=self.out_channels)
...
class ResNet34(nn.Module):
    ...
    def __init__(self, featmap_list: list[str]):
        super().__init__()
        self.features = models.resnet34(weights="DEFAULT")
        self.features = tc.nn.Sequential(*list(self.features.children())[:-2])
        self.features.requires_grad_(False)
        for params in self.features[5].parameters():
            params.requires_grad_(True)
...
class BackboneResNet34(FasterRCNNBackbone):
    def __init__(self, featmap_list: list[str] = ["c1", "c2", "c3", "c4"]):
        super().__init__()
        self.out_channels=256
        self.featmap_list = featmap_list
        self._backbone = ResNet34(featmap_list=featmap_list)
        self._fpn = FeaturePyramidNetwork(in_channels_list=self._backbone.in_channels_list, out_channels=self.out_channels)
