...
class ActivationFunction:
    ...
    @staticmethod
    def leaky_relu(X: tc.Tensor) -> tc.Tensor:
        return tc.nn.functional.leaky_relu(X)
    @staticmethod
...
class LossFunction:
    ...
    def softmax_cross_entropy(
        ...
        """
        class_weight = LossFunction.class_weight
...
class NeuralNetwork:
    def __init__(
        ...
        Args:
            ...
            loss_func (Callable[[tc.Tensor, tc.Tensor], tc.Tensor]): 
        """
        ...
        for i in range(len(layers)):
            ...
            W = tc.randn((m, n), requires_grad=True, dtype=tc.float)
            B = tc.zeros((1, m), requires_grad=True, dtype=tc.float) 
    ...
    def forward(
        ...
        label: tc.Tensor
            ...
            #print(f"A[0] = {input}")
            for i in range(L0):
                input = f[i](input @ W[i].T + B[i])
                # print(f"A[{i + 1}] = {input}")
            loss = loss_func(input, label)
            return loss
    
    def _optimize(self, alpha: float = 0.05):
        L0 = len(self._W)
        with tc.no_grad():
            for i in range(L0):
                # print(f"{i} : {W[i].grad}")
                ...
                self._W[i].grad.zero_()
                self._B[i].grad.zero_()    
