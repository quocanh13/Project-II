...
class FaceVerification(nn.Module):
    ...
    def extract_features(
        self, 
        image: Image.Image
        ) -> tuple[list[tuple[list[int], float]], bool, tc.Tensor]:
        frontal = True
        with tc.no_grad():
        ...
        return detection, frontal, features[0]

    def sample(
        self, 
        sample_image: Image.Image
    ) -> tuple[list[tuple[dict]], bool, Image.Image | None]:
        with tc.no_grad():
    ...
    def verify(
    ...
    ) -> tuple[list[tuple[dict], bool], float]:
        res_1 = self.extract_features(sample_image)
        res_2 = self.extract_features(verify_image)
        
        detection_1, frontal_1, features_1 = res_1
        detection_2, frontal_2, features_2 = res_2
        t = detection_1
        detection_1 = []
        for bbox, score in t:
        ...
        return [(detection_1, frontal_1), (detection_2, frontal_2)], distance.item()